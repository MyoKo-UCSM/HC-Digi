      ...
      this.on('click', function(e) {
        ...
        window.SetUrl = function (items) {
          ...
          allfiles.split(',').forEach(function (item,key) {
            console.log(lfmPtype);
            let imgElement ="";
            if(lfmPtype !== "g"){
              if (target_preview_val === 'holder-gift-image') {
                imgElement =
                    "<div class='" + lfmImageInput + "lfmc" + key + "'>" +
                    "<video src='" + item + "' style='height:" + mh + "rem;' class='lfmimage custom-image-input w-100' controls>" +
                    "<source src='" + item + "' ></video><div><button type='button' onclick='removeImage(\""+lfmImageInput+"\","+key+")' class='btn btn-icon btn-active-light-danger btn btn-danger w-40px h-40px remove-image w-100' style='position: absolute; top: 150px; right: 50px;'><i class='bi bi-trash'></i></button></div></div>";
              } else {
